...
class MeshCache(AbstractRubyCacheHierarchy, AbstractThreeLevelCacheHierarchy):
    ...
    def get_mesh_descriptor(self) -> MeshTracker:
        return self._mesh_descriptor
    ...
    def _setup_ruby_system(self) -> None:
        ...
        self.ruby_system.network = MeshNetwork(
            ruby_system=self.ruby_system, mesh_descriptor=self._mesh_descriptor
        )
    ...
    def _create_dma_tiles(self, board: AbstractBoard) -> None:
        ...
        if not board.has_dma_ports():
            return
        self._has_dma = True
        dma_tile_coordinates = self._mesh_descriptor.get_tiles_coordinates(
            NodeType.DMATile
        )
        self.dma_tiles = [
            DMATile(
            ...
            for dma_id, (dma_tile_coordinate, dma_port) in enumerate(
                zip(dma_tile_coordinates, board.get_dma_ports())
            )
        ]
    ...
    def _setup_cache_block_tracker(self, board: AbstractBoard) -> None:
        ...
        for core in board.get_processor().get_cores():
            if hasattr(core, "generator"):
                ...
                self.cache_block_tracker.addDemandRequestor(core.core)
        ...
        for l3_slice in self._get_all_l3_slices():
