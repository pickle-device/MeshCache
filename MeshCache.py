...
class MeshCache(AbstractRubyCacheHierarchy, AbstractThreeLevelCacheHierarchy):
    def __init__(
        ...
        if len(pickle_device_tile_coordinates) > 0 and not self.support_pickle_device_tile():
            print("MeshCache does not support PickleDeviceTile. However, MeshCacheWithPickleDevice does.")
            exit(1)
    ...
    def _create_memory_tiles(self, board: AbstractBoard) -> None:
        ...
        if not len(mem_tile_coordinates) == len(board.get_mem_ports()):
            assert False, "Different number of memory tiles and number of memory channels."
